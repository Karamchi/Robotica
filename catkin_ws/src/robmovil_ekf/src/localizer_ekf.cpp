...
void robmovil_ekf::LocalizerEKF::makeH(void)
{
  ...
  tf::Point relative_landmark = transform_world_robot.inverse() * correspondence_landmark;//tf::Point(0,0,0);
  ...
  if (relative_landmark.length2() < 0.001)
  {
    ...
    H(1,1) = (-(relative_landmark.getX())) / (relative_landmark).length();
    H(1,2) = (-(relative_landmark.getY())) / (relative_landmark).length();
    
    H(2,1) = ( (relative_landmark.getY())) / (relative_landmark).length2();
    H(2,2) = (-(relative_landmark.getX())) / (relative_landmark).length2();
...
void robmovil_ekf::LocalizerEKF::makeProcess(void)
{
  ...
  x(1) = delta_t * u(1) * cos(x_old(3)) + x_old(1);
  x(2) = delta_t * u(1) * sin(x_old(3)) + x_old(2);
...
bool robmovil_ekf::LocalizerEKF::find_corresponding_landmark(const tf::Point& measured_landmark, tf::Point& corresponding_landmark, float delta_radio)
{
  ...
  {
    /* COMPLETAR */
    float dist = (map_landmarks[i] - measured_landmark).length();
    if (dist < min_distance && dist < delta_radio) {
...
robmovil_ekf::LocalizerEKF::Vector robmovil_ekf::LocalizerEKF::landmark2measure(const tf::Point& landmark)
{
  ...
  /* COMPLETAR */
  tf::Point relative_landmark = transform_world_robot.inverse() * landmark;//tf::Point(0,0,0);
