...
void robmovil_ekf::LandmarkDetector::on_laser_scan(const sensor_msgs::LaserScanConstPtr& msg)
{
  ...
  landmark_points.push_back(cartesian[0]);
  
  for (int i = 1; i < cartesian.size(); i++)
  {
    ROS_INFO_STREAM("cartesian" << cartesian[i].getX() << ' ' << cartesian[i].getY());
    /* COMPLETAR: Acumular, de manera secuencial, mediciones cercanas (distancia euclidea) */
    
    // -------
    if ((cartesian[i] - cartesian[i-1]).length() < LANDMARK_DIAMETER && i < cartesian.size() - 1) {
      ...
      float a = atan2(centroid.getX(), centroid.getY()); // angulo de la recta que conecta al robot con el centroide
      landmark.bearing = a;
